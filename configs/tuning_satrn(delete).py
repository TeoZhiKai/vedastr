                                ...
                                layers=[
                                    ...
                                    ('conv',
                                     dict(type='ConvModule', in_channels=int(hidden_dim / 2), out_channels=hidden_dim,
                                          kernel_size=3,
                                          stride=1, padding=1, norm_cfg=batch_norm)),
                                    ('pool', dict(type='MaxPool2d', kernel_size=(4,2), stride=(4,2), padding=0)),
                                ],
                            ),
                        ),
...
train = dict(
    ...
    resume=dict(checkpoint='D:/FYPtesting/vedastr/vedastr/workdir/tuning_satrn/best_acc.pth')                # for continue training from the previous checkpoint
)
